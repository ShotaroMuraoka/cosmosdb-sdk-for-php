...
final class DeleteConflictCommand implements CommandInterface
{
    private const string METHOD = 'DELETE';
    private const string RESOURCE_TYPE = 'conflicts';
    private const string RESOURCE_LINK = 'dbs/%s/colls/%s/conflicts/%s';
    private const string ENDPOINT = '/dbs/%s/colls/%s/conflicts/%s';
    ...
    public function execute(RequestDtoInterface $request): Result
    {
        ...
        $conflictId = $request->pathParameters['conflictId'];
        $headers = $this->client->authStrategy->getAuthHeaders(
            verb: self::METHOD,
            resourceType: self::RESOURCE_TYPE,
            resourceLink: sprintf(self::RESOURCE_LINK, $dbId, $collId, $conflictId),
        ...
        return $this->client->sender->send(
            method: self::METHOD,
            resourcePath: sprintf(self::ENDPOINT, $dbId, $collId, $conflictId),
            headers: $headers,
            body: $request->body,
        );
    }
